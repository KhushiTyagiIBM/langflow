...
| `LANGFLOW_SANDBOX_MAX_OUTPUT_BYTES` | Integer | `1048576` | Combined stdout and stderr retained from one sandbox run, in bytes (1 MiB). A run that exceeds the limit is stopped instead of consuming unbounded worker memory or returning silently truncated output. |
